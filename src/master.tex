\documentclass[11pt, a4paper, english]{article}
\usepackage[margin=1in]{geometry}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc,url}
\usepackage[english]{babel}
\usepackage{textcomp}
\usepackage{amsmath}
\usepackage{verbatim}
\usepackage[hidelinks]{hyperref}
\usepackage{placeins}
\usepackage{frontpage/ifikompendiumforside}
\usepackage{url}
\usepackage[backend=biber]{biblatex}
\usepackage{hyperref}
\usepackage{csquotes}
\usepackage{graphicx}
\graphicspath{{figures/}}
\usepackage{amssymb}
\usepackage{ upgreek }
\usepackage{listings} 
\usepackage{tikz}
\usepackage{tablefootnote}
\newcommand{\para}{\vspace{1.25\baselineskip}\noindent}


\newcommand{\lhk}[1]{\textcolor{red}{[#1]}}

\addbibresource{bibliography.bib}

\title{Frog: Functions for ontologies}
\subtitle{An Essay}
\author{Marlen Jarholt}
\date{19.01.2021} %endre naar ferdig


\lstdefinelanguage{turtle}
{
    columns=fullflexible,
    keywordstyle=\color{red},
    morekeywords={@prefix,@base,@forSome,@forAll,@keywords},
    morecomment=[l]{\#},
    tabsize=4,
    alsoletter={-?}, % allowed in names
    morecomment=[s][\color{blue}]{<}{>},
    basicstyle=\ttfamily\color{black},
    %numberstyle=\color{black},
    morestring=[b][\color{black}]\",
    backgroundcolor=\color{white},
}

\newcommand{\greencheck}{{\color{green}\checkmark}}
\newcommand{\redx}{{\color{red}X}}

\begin{document}

\ififorside

\clearpage

\tableofcontents

\clearpage

\lhk{Add an introduction outlining what the essay is about}
\input{sections/functionalPrograming.tex}
\input{sections/SemanticWebAndRDF.tex}
\input{sections/frog.tex}

\clearpage
\printbibliography

\end{document}
